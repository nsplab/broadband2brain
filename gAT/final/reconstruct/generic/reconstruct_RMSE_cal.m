function [t, c, sigma, elapsed_time] = reconstruct_RMSE_cal(y, T, args, prev_data)
...
% Adapted from reconstruct_ker.m with a few changes

% Author: Alex Wein
% Date: Aug, 2011

% Seems to update t_k, c, sigma
...
for iter = 1:numiter
    ...
    for k = 1 : K,  % Estimate t_k
        ...
        plot([t(k), t(k)], ylim, 'r');
        xlabel('t_k');
        title(['iter = ' num2str(iter) '  k = ' num2str(k)]);
        %}
        % END TESTING
        ...
        %    z(n) = d;
        %end
        z = h(repmat((1 : L)', [1 K])*T_s - repmat(t', [L 1]), args) * c;

        lambda_0 = (y-z)' * cov_inv * (y-z);
        lambda_1 = 2 * (y-z)' * cov_inv * mu;
        lambda_2 = mu' * cov_inv * mu;
        sigma = lambda_1 / (2 * lambda_2);
        %%}
...
% TESTING: plot convergence of params (need to uncomment sections above as
% well)
%{
figure;
