function [t, c, sigma, elapsed_time] = reconstruct_ker(y, T, args, prev_data)
...
for iter = 1:numiter
    ...
    if debug == 1
        if K == 2
            ...
            plot(sig_vec, tau_stuff * max(L_vec) / max(tau_stuff), 'g');
            %plot(sig_vec, exp_arg, 'y');
            plot([c(this_k) c(this_k)], ylim, 'r');
            xlabel('c_k');
            title('c');
            legend('L (likelihood)', 'tau (RMSE)', 'c (chosen value)');
        end
    end
    %}
    % END TESTING
    ...
    for k = 1 : K,  % Estimate t_k
        ...
        for kprime = 1 : K,
            if kprime ~= k,
                for n = 1 : L,
                    ...
                    % TESTING: count discrepancies between matrix and
                    % direct calc
                    %{
                    using_mat = h_mat(t_ind(kprime), n);
                    direct_calc = h(n * T_s - t(kprime), args);
                    diff = using_mat - direct_calc;
                    if diff ~= 0
                        diff
                    end
                    %}
        ...
        like = prob_t*c(k) + y'*cov_inv*y;  % testing, should match tau_stuff *** but doesn't!!
        ...
        for kprime = 1 : K
            if kprime ~= k
                % TESTING *** (remove impose delta) -- set rem_ind = [];
        ...
        % TESTING: plot distribution (remove impose delta for this to work)
        %{
        tau_stuff = [];
        tau_by_parts = [];
        exp_arg = [];
        ...
        for i = 1 : length(t_sampled)
            ...
            for n = 1 : L,
                ...
                for k_prime = 1 : K,
                    if k == k_prime
                        ...
                        d = d + c(k_prime) * h(n*T_s - t(k_prime), args);  % bug fixed, was c(k)
                    end
                end
                mu_vec(n) = d;
            end
            mu_vec = mu_vec + sigma / sqrt(2 * pi);
            tau_stuff(i) = (y-mu_vec)' * cov_inv * (y-mu_vec);
            tau_by_parts(i) = y'*cov_inv*y - 2*mu_vec'*cov_inv*y + mu_vec'*cov_inv*mu_vec;
            exp_arg(i) = -1/(2*sigma^2 * (1/2-1/(2*pi)) * delta_t) * ((y-mu_vec)' * cov_inv * (y-mu_vec));
            L_vec(i) = 1/sigma^L * exp(-1/(2*sigma^2 * (1/2-1/(2*pi)) * delta_t) * ((y-mu_vec)' * cov_inv * (y-mu_vec)));
            for n = 1 : L
                beta_tilde(n) = h(n*T_s - t_val, args);
                alpha(n) = y(n) - sigma/sqrt(2*pi);
                for kprime = 1 : K
                    if kprime ~=k
            ...
            B_const_2 = beta_tilde'*cov_inv*beta_tilde;
            %B_const(i) - B_const_2  % B_const is ok
            tau_stuff_2(i) = (alpha - c(k)*beta_tilde)'*cov_inv*(alpha - c(k)*beta_tilde);
        end
        ...
        for i = 1 : length(t_fine)
            ...
            mu_vec = mu_vec + sigma / sqrt(2 * pi);
            tau_stuff_fine(i) = (y-mu_vec)' * cov_inv * (y-mu_vec);
        end
        figure;
        hold on;      
        plot(t_sampled, prob_t * max(tau_stuff) / max(prob_t), 'b.-')
        plot(t_sampled, tau_stuff, 'r.-');
        plot(t_fine, tau_stuff_fine, 'g');
        %plot(t_sampled, like, 'g.-');
        %plot(t_sampled, tau_stuff_2, 'k.-');
        plot([t(k) t(k)], [min(tau_stuff) max(tau_stuff)], 'r');
        legend('prob t', 'tau stuff', 'tau stuff fine', 't_k');
        title(['iter ' int2str(iter) '  k = ' int2str(k)]);
        % Test if prob_t and tau_stuff are consistent
        % ** There are discrepancies here!
        % prob_t is consistent with tau_stuff_2 but not with tau_stuff
        % (fixed now)
        bad_count = 0;
        for itervar = 1 : 100
            i_1 = ceil(rand()*length(prob_t));
            i_2 = ceil(rand()*length(prob_t));
            if (prob_t(i_1) - prob_t(i_2))*(tau_stuff(i_1) - tau_stuff(i_2)) < 0
            ...
            for n = 1 : L,
                ...
                for k_prime = 1 : K,
                    if k == k_prime
                        ...
                        d = d + c(k_prime) * h(n*T_s - t(k_prime), args);
                    end
                end
                mu_vec(n) = d;
            end
    ...
    if debug == 1
        if K == 2
            sig_step = 0.5;
            num_plot = 20;
            sig_vec = sigma - sig_step * num_plot : sig_step : sigma + sig_step * num_plot;
            L_vec = zeros(size(sig_vec));
            mu_vec = zeros(L, 1);    
            for i = 1 : length(sig_vec)
                for n = 1 : L,
            ...
            plot([sigma, sigma], ylim, 'r');
            title('sigma');
            legend('L (likelihood)', 'tau (RMSE)', 'sigma (chosen value)');
        end
    end
    %}
    % END TESTING
