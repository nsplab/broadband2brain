function [setup_obj] = setup_GML_modified(T, window, args)
% Precomputes values for generic kernel
% args.h is function handler for kernel
% args.h_args is args for kernel

% MODIFICATION: samples are now taken at linspace(T/L, T, L) instead of linspace(0, T, L)

L = args.L;
h = args.h;
J = args.J;

ns = linspace(T/L, T, L);  % points at which samples are taken
