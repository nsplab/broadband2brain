...
est_rej_thres_sinc(data_set, electrodes, cond, cond, T, L, K, B, delta_t, numiter, t_resolution, tolerance);

for electrode = electrodes

    % FRI_sinc_diode spikes
    [t, c, t1, sigma] = run_FRI_sinc_diode(data_set, [electrode], start_time, end_time, cond, start_time, T, L, K, B, delta_t, numiter, t_resolution);
    t = t{1};
    c = c{1};
    t1 = [t1 t1(end)+T];
    sigma = sigma{1};
    ...
    %stem(t, c*scale, 'Color', [0.5, 0.5, 0.5]);  % Reconstructed spikes
    stem(keep_t, keep_c*scale, 'k');  % Reconstructed spikes
    stem(rej_t, rej_c*scale, 'Color', [0.5, 0.5, 0.5]);  % Rejected spikes
    plot(t1, ones(size(t1))*-20, 'b.');  % Sampling times
    plot(spikes, ones(size(spikes))*-10, 'k.');  % Real spikes
    ...
    leg = legend('data', 'reconstructed spikes', 'rejected spikes', 'sampling times', 'real spikes', 'threshold', 'true positives', 'false positives', 'false negatives');
