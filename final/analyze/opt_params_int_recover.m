% Loads results from opt_params_int so that they can be replotted without recomputing
...
for data_set = data_sets
    ...
    for elec = electrodes
        ...
        Ts = eval([obj '.Ts']);
        Ls = eval([obj '.Ls']);
        err_rate = eval([obj '.err_rate']);
        fp_rate = eval([obj '.fp_rate']);
        fn_rate = eval([obj '.fn_rate']);
        min_err = eval([obj '.min_err']);
        fp = eval([obj '.fp']);
        fn = eval([obj '.fn']);
        bestT = eval([obj '.bestT']);
        bestL = eval([obj '.bestL']);
        ind_T = eval([obj '.ind_T']);
        ind_L = eval([obj '.ind_L']);
        ...
        % Save figure
        saveas(fig, ['~/Desktop/optrec/opt_' method_name '_' int2str(data_set) '_' int2str(elec) '.pdf']);
