...
% Different from convolve_setup.m because samples taken at both endpoints of interval
...
T_s = T / L;
t_y = linspace(0, T, L);  % ** line changed

conv_mat = zeros(L, length(time));
for j = 1 : L
    for k = 1 : length(time)
        conv_mat(j, k) = h(t_y(j) - time(k), args) * get_dt();
    end
end
